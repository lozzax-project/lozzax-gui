; Lozzax Mars GUI Wallet Installer for Windows
; Copyright (c) 2021-2022, The Lozzax Project
; Copyright (c) 2017-2020, The Monero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\lozzax-wallet-gui.exe")

[Setup]
AppName=Lozzax GUI Wallet
...
DefaultDirName={commonpf}\Lozzax GUI Wallet
DefaultGroupName=Lozzax GUI Wallet
UninstallDisplayIcon={app}\lozzax-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Lozzax Developer Community
AppPublisherURL=https://docs.lozzax.xyz
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Lozzax files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Lozzax.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Lozzax GUI wallet exe and guide
Source: "bin\lozzax-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\lozzax-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Lozzax CLI wallet
Source: "bin\extras\lozzax-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lozzax wallet RPC interface implementation
Source: "bin\extras\lozzax-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lozzax daemon
Source: "bin\lozzaxd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lozzax daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "lozzax-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Lozzax blockchain utilities
Source: "bin\extras\lozzax-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\lozzax-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\lozzax-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitlozzax');
  s := 'The default folder to store the Lozzax blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\lozzax-project\lozzax-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Lozzax default location
...
begin
  Result := BlockChainDir('') + '\bitlozzax.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "lozzax-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Lozzax daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\lozzaxd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\lozzax-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Lozzax GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\lozzax-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\lozzax-gui-wallet-guide.pdf"; IconFilename: "{app}\lozzax-wallet-gui.exe"
...
Name: "{group}\Utilities\Lozzax Daemon"; Filename: "{app}\lozzaxd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\lozzax-wallet-cli.exe"; WorkingDir: "{userdocs}\Lozzax\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Lozzax\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\lozzax-wallet-gui\lozzax-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\lozzax-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM lozzaxd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\lozzax-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\lozzax-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\lozzax-project\lozzax-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\lozzax-project\lozzax-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "lozzax:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "lozzax://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "lozzax"; ValueType: "string"; ValueData: "URL:Lozzax Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "lozzax"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "lozzax\DefaultIcon"; ValueType: "string"; ValueData: "{app}\lozzax-wallet-gui.exe,0"
Root: HKCR; Subkey: "lozzax\shell\open\command"; ValueType: "string"; ValueData: """{app}\lozzax-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "lozzaxseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "lozzaxseed://a.b.c.d:port"
Root: HKCR; Subkey: "lozzaxseed"; ValueType: "string"; ValueData: "URL:Lozzax Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "lozzaxseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "lozzaxseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\lozzax-wallet-gui.exe,0"
Root: HKCR; Subkey: "lozzaxseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\lozzax-wallet-gui.exe"" ""%1"""
