...
## Table of Contents
  ...
  * [Installing the  Lozzax GUI from a package](#installing-the-lozzax-gui-from-a-package)
  * [Compiling the Lozzax GUI from source](#compiling-the-lozzax-gui-from-source)
...
- Web: [lozzax.xyz](https://lozzax.xyz)
- Mail: [dev@lozzax.xyz](mailto:dev@lozzax.xyz)
- Github: [https://github.com/lozzax-project/lozzax-gui](https://github.com/lozzax-project/lozzax-gui)
- Discord: [Discord](https://discord.gg/BssSsnEnvs)
...
**Privacy:** Lozzax uses a cryptographically sound system to allow you to send and receive funds without your transactions being easily revealed on the blockchain (the ledger of transactions that everyone has). This ensures that your purchases, receipts, and all transfers remain absolutely private by default.
...
This is the GUI for the [core Lozzax implementation](https://github.com/lozzax-project/lozzax). It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of Lozzax that uses the protocol and network in a compatible manner.
...
## Compiling the Lozzax GUI from source

*Note*: Qt 5.9.7 is the minimum version required to build the GUI.

*Note*: Official GUI releases use lozzax-wallet-gui from this process alongside the supporting binaries (lozzaxd, etc) from the [CLI deterministic builds](https://github.com/lozzax-project/lozzax/blob/master/contrib/gitian/README.md).
...
2. Clone the repository
   ```
   git clone --branch master --recursive https://github.com/lozzax-project/lozzax-gui.git
   ```
   \* `master` - replace with the desired version tag (e.g. `v0.17.1.9`) to build the release binaries.
3. Prepare build environment
   ```
   cd lozzax-gui
   docker build --tag lozzax:build-env-windows --build-arg THREADS=4 --file Dockerfile.windows .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <LOZZAX_GUI_DIR_FULL_PATH>:/lozzax-gui -w /lozzax-gui lozzax:build-env-windows sh -c 'make depends root=/depends target=x86_64-w64-mingw32 tag=win-x64 -j4'
   ```
   \* `<LOZZAX_GUI_DIR_FULL_PATH>` - absolute path to `lozzax-gui` directory  
   \* `4` - number of CPU threads to use
5. Lozzax GUI Windows static binaries will be placed in  `lozzax-gui/build/x86_64-w64-mingw32/release/bin` directory
...
3. Prepare build environment
   ```
   cd lozzax-gui
   docker build --tag lozzax:build-env-linux --build-arg THREADS=4 --file Dockerfile.linux .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <LOZZAX_GUI_DIR_FULL_PATH>:/lozzax-gui -w /lozzax-gui lozzax:build-env-linux sh -c 'make release-static -j4'
   ```
   \* `<LOZZAX_GUI_DIR_FULL_PATH>` - absolute path to `lozzax-gui` directory  
   \* `4` - number of CPU threads to use
5. Lozzax GUI Linux static binaries will be placed in  `lozzax-gui/build/release/bin` directory
6. (*Optional*) Compare `lozzax-wallet-gui` SHA-256 hash to the one obtained from a trusted source
   ```
   docker run --rm -it -v <LOZZAX_GUI_DIR_FULL_PATH>:/lozzax-gui -w /lozzax-gui lozzax:build-env-linux sh -c 'shasum -a 256 /lozzax-gui/build/release/bin/lozzax-wallet-gui'
   ```
   \* `<LOZZAX_GUI_DIR_FULL_PATH>` - absolute path to `lozzax-gui` directory  
...
2. Clone the repository
   ```
   git clone --recursive https://github.com/lozzax-project/lozzax-gui.git
   ```
3. Prepare build environment
   ```
   cd lozzax-gui
   docker build --tag lozzax:build-env-android --build-arg THREADS=4 --file Dockerfile.android .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <LOZZAX_GUI_DIR_FULL_PATH>:/lozzax-gui -e THREADS=4 lozzax:build-env-android
   ```
   \* `<LOZZAX_GUI_DIR_FULL_PATH>` - absolute path to `lozzax-gui` directory  
   \* `4` - number of CPU threads to use
5. Lozzax GUI APK will be placed in  `lozzax-gui/build/Android/release/android-build` directory
6. Deploy
   * Using ADB (Android debugger bridge)
     - [Enable adb debugging on your device](https://developer.android.com/studio/command-line/adb.html#Enabling)
      * Connect your device with USB and install Lozzax GUI APK with adb:
      ```
      adb install build/Android/release/android-build/lozzax-gui.apk
      ```
      * Troubleshooting:
      ```
      adb devices -l
   ...
   * Using a web server
      ```
      mkdir /usr/tmp
      cp build/Android/release/android-build/lozzax-gui.apk /usr/tmp
...
3. Install [lozzax](https://github.com/lozzax-project/lozzax) dependencies:
...
5. Grab an up-to-date copy of the lozzax-gui repository
...
The Lozzax GUI on Windows is 64 bits only; 32-bit Windows GUI builds are not officially supported anymore.

1. Install [MSYS2](https://www.msys2.org/), follow the instructions on that page on how to update system and packages to the latest versions

2. Open an 64-bit MSYS2 shell: Use the *MSYS2 MinGW 64-bit* shortcut, or use the `msys2_shell.cmd` batch file with a `-mingw64` parameter

3. Install MSYS2 packages for Lozzax dependencies; the needed 64-bit packages have `x86_64` in their names
    ...
    You find more details about those dependencies in the [Lozzax documentation](https://github.com/lozzax-project/lozzax). Note that that there is no more need to compile Boost from source; like everything else, you can install it now with a MSYS2 package.
